  ...
  case tokens {
    ...
    [head, ..rest] -> {
      case dict.get(f.words, head) {
        ...
        Error(Nil) -> {
          let result = process_token(f, head)
          case result {
            Error(e) -> Error(e)
...
// list : de stack
// dict : verzameling woorden en hun waarden
//        let op: een custom word kan een built-in overschrijven
//        let op: een custom word kan andere custom/built-in words bevatten
// 
// acties :
// - wat is type Forth precies? relatie met stack impl?
// - wat is de exacte functie van format_stack?
// - wat is de exacte functie van eval?
// - words parsen voordat ze op stack gaan of in eval?
// - mag stack nog inhoud bevatten na een geslaagde eval?
// - input naar lowercase
// - tokenize input
// - parse input met custom words
// - parse input met nested custom words
// - rekenen: +, -, * en /
// - DUP
// - DROP
// - SWAP
...
// - InvalidWord
// - UnknownWord
// - onderscheid unaire en binaire operaties
// - word heeft naam met special characters (bijv. euro)
