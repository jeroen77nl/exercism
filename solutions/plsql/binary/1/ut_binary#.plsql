CREATE OR REPLACE PACKAGE binary# IS
   FUNCTION to_decimal(p_binary varchar2) 
      RETURN pls_integer;
...
CREATE OR REPLACE PACKAGE BODY binary# IS
   ...
   AS
      ...
      l_exponent pls_integer;
      l_bit
   BEGIN 
      for i in 1 .. length(p_binary) loop
         l_exponent := length(p_binary) - i;
         l_bit := to_number(substr(p_binary, 1));
         l_decimal := l_decimal + l_bit * power(2, l_exponent);
      end loop;    
      RETURN l_decimal;
   END hello;
END binary#;
