...
class RunLengthEncoding {
    ...
    String encode(String data) {
        ...
        while (startPos < data.length()) {
            int count = countRepeatedChars(data, startPos);
            if (count >= 2) {
                result.append(count);
            }
    ...
    private int countRepeatedChars(String data, int startPos) {
        int count = 1;
        for (int i = startPos + 1;
             i < data.length() && data.charAt(i) == data.charAt(startPos);
             i++) {
            count++;
        }
        return count;
