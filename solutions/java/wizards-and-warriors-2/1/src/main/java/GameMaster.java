public class GameMaster {
    ...
    public String describe(TravelMethod travelMethod) {
        String byOn = travelMethod == TravelMethod.HORSEBACK ? "on" : "by";
        return "You're traveling to your destination %s %s."
            .formatted(byOn, travelMethod.name().toLowerCase());
    }

    public String describe(Character character, Destination destination, TravelMethod travelMethod) {
        return "%s %s %s"
            .formatted(describe(character), describe(travelMethod), describe(destination));
    }
